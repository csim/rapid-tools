...
{
    public class ListNodeTag : NodeTag
    {
        public ListNodeTag(TreeNode node, DTE2 applicationObject)
        {
            _node = node;
        ...
        public override ContextMenu GetContextMenu()
        {
            ...
            MenuItem _viewsMenuItem = _contextMenu.MenuItems.Add("Views");

            List<string> _viewNames = new List<string>(ServiceInstance.Views(SiteUrl, WebGuid, Guid));

            _viewsMenuItem.MenuItems.Add("List Schema", delegate(object sender, EventArgs e)
            {
                string _schemaPath = string.Concat(Environment.GetFolderPath(Environment.SpecialFolder.ApplicationData), "\\SPDeploy\\Workspace\\Lists\\", Text, ".xml");

                Domain.Utilties.Functions.EnsurePath(_schemaPath);

                File.WriteAllText(_schemaPath, _listsWebService.GetList(Text).OuterXml);

                ApplicationObject.ItemOperations.OpenFile(_schemaPath, EnvDTE.Constants.vsViewKindTextView);
            });

            foreach (string viewName in _viewNames)
            {
                StringBuilder _viewPathBuilder = new StringBuilder();
                _viewPathBuilder.Append(Environment.GetFolderPath(Environment.SpecialFolder.ApplicationData));
                _viewPathBuilder.Append("\\SPDeploy\\Workspace\\");
                _viewPathBuilder.Append(SiteUrl);
                _viewPathBuilder.Append("\\");
                _viewPathBuilder.Append(Node.Parent.Text);
                _viewPathBuilder.Append("\\");
                _viewPathBuilder.Append(Text);
                _viewPathBuilder.Append("\\");
                _viewPathBuilder.Append(viewName);
                _viewPathBuilder.Append(".xml");


                MenuItem _viewMenuItem = _viewsMenuItem.MenuItems.Add(viewName);
                _viewMenuItem.Tag = viewName;
                if (File.Exists(_viewPathBuilder.ToString()))
                {
                    _viewMenuItem.MenuItems.Add("Open", delegate(object sender, EventArgs e)
                    {
                        Domain.Utilties.Functions.OpenVsItem(_viewPathBuilder.ToString(), ApplicationObject);
                    });
                    _viewMenuItem.MenuItems.Add("Check In", delegate(object sender, EventArgs e)
                    {
                        string str2 = ((MenuItem)((MenuItem)sender).Parent).Text;
                        ServiceInstance.UpdateViewSchema(SiteUrl, WebGuid, Guid, ((MenuItem)((MenuItem)sender).Parent).Text, File.ReadAllText(_viewPathBuilder.ToString()));
                        File.Delete(_viewPathBuilder.ToString());
                    });
                    _viewMenuItem.MenuItems.Add("Discard Check Out", delegate(object sender, EventArgs e)
                    {
                        File.Delete(_viewPathBuilder.ToString());
                    });
                }
                ...
                {
                    string viewSchema = ServiceInstance.GetViewSchema(SiteUrl, WebGuid, Guid, viewName);

                    _viewMenuItem.MenuItems.Add("Preview", delegate(object sender, EventArgs e)
                    {
                        string tempPath = Domain.Utilties.Functions.GetRandomTempPath();
                        File.WriteAllText(tempPath, viewSchema);
                        ApplicationObject.ItemOperations.OpenFile(tempPath, EnvDTE.Constants.vsViewKindTextView);
                    });
                    _viewMenuItem.MenuItems.Add("Check Out", delegate(object sender, EventArgs e)
                    {
                        Domain.Utilties.Functions.EnsurePath(_viewPathBuilder.ToString());
                        File.WriteAllText(_viewPathBuilder.ToString(), viewSchema);
                        ApplicationObject.ItemOperations.OpenFile(_viewPathBuilder.ToString(), EnvDTE.Constants.vsViewKindTextView);
